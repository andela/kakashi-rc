...
Template.staticPageLayout.helpers({
  ...
  // file stats
  fileStats() {
    const drafts = StaticPage.find({
      $and: [{
        shopId: Reaction.getShopId(),
        status: "draft"
      }]
    }).count();
    const published = StaticPage.find({
      $and: [{
        shopId: Reaction.getShopId(),
        status: "publish"
      }]
  ...
  // fetch all pages
  getAllPages() {
    const pages = StaticPage.find().fetch();
    pages.forEach(page => {
...
Template.staticPageLayout.events({
  ...
  "click [data-event-action=edit]": function () {
    const pageId = this._id;
    const currentPage = StaticPage.findOne({ _id: this._id });
